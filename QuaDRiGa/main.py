...
def generate_real_samples_with_labels_Rayleigh(h_dataset, number=100):
    ...
    """
    # 1. 隨機選取通道係數 h
    # 從 h_dataset 中隨機抽取 number 個樣本
    idx = np.random.randint(0, len(h_dataset), number)
    h = h_dataset[idx]
    
    # 2. 產生隨機 16-QAM 符號 (對應主程式底部的 mean_set_QAM)
    qam_16 = np.asarray([-3-3j, -3-1j, -3+1j, -3+3j, -1-3j, -1-1j, -1+1j, -1+3j,
                         1-3j, 1-1j, 1+1j, 1+3j, 3-3j, 3-1j, 3+1j, 3+3j], dtype=np.complex64)
    x_idx = np.random.randint(0, len(qam_16), number)
    x = qam_16[x_idx]
    
    # 3. 模擬接收訊號 y = hx + n
    # 根據你主程式底部的繪圖設定，雜訊變異數 (variance) 是 0.03
    noise_real = np.random.normal(0, np.sqrt(0.03), number)
    noise_imag = np.random.normal(0, np.sqrt(0.03), number)
    noise = noise_real + 1j * noise_imag
    
    y = h * x + noise
    
    # 將 y 轉為 [實部, 虛部] 的二維矩陣 (N, 2)
    received_data = np.hstack((np.real(y).reshape(-1, 1), np.imag(y).reshape(-1, 1)))
    
    # 4. 構建條件向量 (Conditioning vector)
    # 維度是 4 (condition_dim = 4)，包含 [發送訊號實部, 發送訊號虛部, 通道實部, 通道虛部]
    # 並且依照主程式的邏輯除以 3 做正規化
    x_r = np.real(x).reshape(-1, 1)
    x_i = np.imag(x).reshape(-1, 1)
    h_r = np.real(h).reshape(-1, 1)
    h_i = np.imag(h).reshape(-1, 1)
    
    conditioning = np.hstack((x_r, x_i, h_r, h_i)) / 3.0
...
if checkpoint:
    ...
    print(f"succeeded in loading checkpoint: {checkpoint}, model memory has been restored!")
else:
