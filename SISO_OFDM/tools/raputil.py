...
def MMSE_CE(Y, pilotValue, pilotCarriers, K, P, h, SNR):
    # 1. 計算導頻位置上的 LS 估計 (Least Squares Estimate)
    H_LS_p = Y[pilotCarriers] / pilotValue
    
    # 2. 利用真實信道 h 計算理想的功率延遲分佈 (Power Delay Profile, PDP)
    # 在實際應用中無法預知 h，但作為演算法比較的 Baseline，通常會使用真實 PDP
    pdp = np.abs(h)**2
    Ph = np.sum(pdp) # 信道總功率
    
    # 將時域協方差矩陣對角化並補零至長度 K
    pdp_padded = np.pad(pdp, (0, K - len(pdp)), 'constant')
    R_hh = np.diag(pdp_padded)
    
    # 產生未歸一化的 KxK DFT 矩陣 (對應 np.fft.fft)
    F_unnorm = np.fft.fft(np.eye(K))
    
    # 計算頻域的信道協方差矩陣 R_HH = F * R_hh * F^H
    R_HH = F_unnorm @ R_hh @ np.conj(F_unnorm).T
    
    # 提取與導頻位置相對應的子矩陣
    R_HpHp = R_HH[np.ix_(pilotCarriers, pilotCarriers)]
    R_HHp = R_HH[:, pilotCarriers]
    
    # 3. 計算雜訊變異數項
    # 根據 OFDM 理論推導，LS 估計上的雜訊變異數約為 Ph / SNR_linear
    snr_linear = 10**(SNR / 10.0)
    noise_var_LS = Ph / snr_linear
    
    # 4. 計算 MMSE 權重矩陣 (Weight Matrix) 與最終信道估計 (Channel Estimation)
    # 公式: W_MMSE = R_HHp * inv(R_HpHp + noise_var_LS * I)
    I = np.eye(P)
    W_MMSE = R_HHp @ np.linalg.inv(R_HpHp + noise_var_LS * I)
    
    # 乘上 LS 估計值得到全頻段的 MMSE 估計
    H_MMSE = W_MMSE @ H_LS_p
