...
def build_ce_dnn(K, SNR, savefile, learning_rate=1e-3, training_epochs=2000, batch_size=50, nh1=500, nh2=250, test_flag=False, cp_flag=True):
    ...
    nn_input = tf.placeholder(tf.float32, [None, n_input], name='nn_input') # YOUR CODE HERE 1
    H_true = tf.placeholder(tf.float32, [None, n_output], name='H_true')    # YOUR CODE HERE 2    # label

    dense1 = Dense(units=nh1, activation='relu')                            # YOUR CODE HERE 3
    dense2 = Dense(units=nh2, activation='relu')                            # YOUR CODE HERE 4
    output_layer = Dense(units=n_output, activation='linear')               # YOUR CODE HERE 5
    ...
    loss_ = tf.reduce_mean(tf.square(H_true - H_out))                       # YOUR CODE HERE 9
