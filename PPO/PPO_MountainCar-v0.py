...
#import gym
import gymnasium as gym # gymnasium 是 gym 的升級版本，API 有些微調整
import torch
...
class PPO():
    ...
    def save_param(self):
        torch.save(self.actor_net.state_dict(), '../param/net_param/actor_net' + str(int(time.time())) +'.pkl')
        torch.save(self.critic_net.state_dict(), '../param/net_param/critic_net' + str(int(time.time())) +'.pkl')
    ...
    def update(self, i_ep):
        ...
        for i in range(self.ppo_update_time):
            for index in BatchSampler(SubsetRandomSampler(range(len(self.buffer))), self.batch_size, False):
                ...
                # ─── YOUR CODE HERE ──────────────────────────────────────────── #
                # 1. Calculate the importance ratio
                # 儲存的是原始機率，直接相除即可得到 ratio
                ratio = action_prob / old_action_log_prob[index]

                # 計算 PPO 的 Clipped Surrogate Objective
                surr1 = ratio * advantage
                surr2 = torch.clamp(ratio, 1 - self.clip_param, 1 + self.clip_param) * advantage
                
                # 2. Update actor network
                # 希望最大化 objective，但在 PyTorch 中 optimizer 是做最小化
                # 所以加上負號轉換為 loss
                ...
                # 加上 Gradient Clipping 避免更新的 step 過大
                nn.utils.clip_grad_norm_(self.actor_net.parameters(), self.max_grad_norm)
                self.actor_optimizer.step()

                # 3. Update critic network
                # Critic 的目標是讓 V(s) 逼近 Gt
...
def main():
    ...
    for i_epoch in range(1000):
        # Gymnasium API：reset 回傳 (observation, info)
        state, info = env.reset(seed=seed if i_epoch == 0 else None)
        if render: env.render()

        for t in count():
            action, action_prob = agent.select_action(state)
            
            # Gymnasium API：step 回傳 5 個值
